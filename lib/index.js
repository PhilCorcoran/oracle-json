    ...
    return function(req,res,next){
        ...
        function handle_result_err(err,results){
            ...
            if (err) {
                ...
                if(dbRequest.noRespond){
                    res.locals.data=JSON.parse(results.returnParam);
                    if(res.locals.data.status && res.locals.data.status.toUpperCase()!='OK'){
                        console.log('Database Error:%s',JSON.stringify(res.locals.data));
