...
OJ.prototype.createPool = function(callback){
  ...
  oracle.createPool(this.poolSettings, function (err, pool) {
    if (err) {
        ...
        if(self.poolSettings.password) {
           self.poolSettings.password = "***";
        }
        console.log("failed to create connection pool on startup, exiting process", err, self.poolSettings);
    ...
    self.getConnection(function (conn) {
        conn.test(function (err) {
            if (err) {
                console.log('testConnection() Error:%s', err.toString());
                process.exit(1);
            }
            conn.release();
            callback();
...
function noData (dbRequest, res, next) {
    if (dbRequest.output) {
        logError('No Data returned, output=true');
