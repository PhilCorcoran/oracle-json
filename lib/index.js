...
function OJ(options) {
    ...
    debug('maxProcedures:%s', options.maxProcedures);
...
OJ.prototype.reconnect = function(next) {
    ...
    try {
        ...
        if (self.retries > self.maxRetries) {
            debug("reconnect() maximum number of retries(%s) exceeded by attempts(%s). exiting process", self.maxRetries, self.retries);
            process.exit(1);
        }
