...
OJConn.prototype.execute = function(dbRequest, inputData, callback) {
    ...
    function handle_result(err, results) {
        ...
        if (err) {
            ...
            var output = results.outBinds[0];
            return processResults(output, dbRequest, callback);
...
function fetchRowsFromRS(resultSet, numRows, callback, results){
      ...
      function (err, rows)
      {
          if (err) {
              ...
              if (rows.length === numRows)  // might be more rows
                  fetchRowsFromRS(resultSet, numRows, callback, results);
              else  {
...
function processResults (output, dbRequest, callback){
    if (dbRequest.outputType==='CURSOR' || dbRequest.query){
        return fetchRowsFromRS(output, dbRequest.numRows, function(err, rows){
            if(err){
                return callback(err);
            }
            ...
            var result = {};
            result[dbRequest.resultName || 'rows'] = rows;
